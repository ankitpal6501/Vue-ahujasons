  ...
  <div id="main-container">
      ...
      <div id="functionality-grp">
        <div id="div1">
          <button @click="hideFilter" id="btn">
            <img
              src="https://ahujasons.com/images/el-images/filter-icon.svg"
              alt=""
            />
        ...
        <div id="div2">
          <div id="all-collected-filter">
            <div
              v-for="option in selectedFiletrs"
              :key="option.value"
              id="selected-filetrs"
            >
              <span>{{ option.value }}</span>
              <svg
                v-on:click="stopFilter(option)"
          ...
          <select
            v-model="sort_by"
            @change="changeProduct(selectedFiletrs, sort_by, pageNo)"
            name=""
            id="btn2"
          >
    ...
    <div id="main-middel">
      <div v-if="showFilter" id="filter-type">
        ...
        <Filter-array
          :filters="filterTypes"
          :selectedFiletrs="selectedFiletrs"
          v-on:updateProductList="changeProduct($event, '', pageNo)"
        />
      </div>
    ...
    <div>
      <MainFooter
        :count="items"
        v-on:changePage="changeProduct(selectedFiletrs, sort_by, $event)"
      />
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async changeProduct(event, sort_by, pageNo) {
      ...
      if (response.status == 200) {
        ...
        if (this.sortTypes.length === 0) {
          this.sortTypes = [
            ...data.result.sort,
            { code: "lth", label: "Price: Low to High" },
            { code: "htl", label: "Price: High to Low" },
          ];
    ...
    hideFilter() {
      if (this.showFilter !== false) {
        ...
        document.querySelector("#main-middel").style.gridTemplateColumns =
          "100%";
